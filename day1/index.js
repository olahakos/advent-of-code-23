...
const getNumbersOut = (data) => {
    ...
    dataArr.forEach((line) => {
        ...
        replacedLineFront.split("").forEach((char) => {
            if (isNumber(char)) numbArrFront.push(char);
        });
        replacedLineBack.split("").forEach((char) => {
            if (isNumber(char)) numbArrBack.push(char);
        });
        if (numbArrFront[0]) total += parseInt(numbArrFront[0]) * 10;
        if (numbArrBack[numbArrBack.length - 1])
            total += parseInt(numbArrBack[numbArrBack.length - 1]);
    });
    return total;
};
